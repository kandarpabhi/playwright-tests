...
test('Checkout Process and Order Placement', async ({ page }) => {
    ...
    // Add a short delay to allow login process to complete
    await page.waitForTimeout(2000); // 2 seconds delay after login
    ...
    // Step 8: Verify if the address details have the correct name
    const fullNameText = await checkoutPage.getFullName();

    // Assert that the full name contains "Mr. Abhishek Kandarp"
    expect(fullNameText.trim()).toContain('Mr. Abhishek Kandarp'); // Move the assertion here

    // Add a short delay to ensure the page is fully loaded before scrolling
